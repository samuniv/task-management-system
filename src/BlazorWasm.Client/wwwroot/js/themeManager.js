...
window.themeManager = {
  ...
  toggleHighContrast: function () {
    const currentEffective = this.getEffectiveTheme();
    const newTheme =
      currentEffective === "high-contrast" ? "auto" : "high-contrast";
    this.setTheme(newTheme);
    return newTheme;
  },
  ...
  updateThemeToggleButton: function () {
    ...
    toggleButtons.forEach((button) => {
      ...
      button.setAttribute(
        "aria-pressed",
        (effectiveTheme !== "light").toString()
      );
      button.setAttribute(
        "aria-label",
        `Current theme: ${currentTheme.label}. Click to change theme.`
      );
      ...
      if (chart.data && chart.data.datasets) {
        chart.data.datasets.forEach((dataset, index) => {
          if (
            ...
            dataset.backgroundColor =
              colors.datasets.primary[index % colors.datasets.primary.length];
            dataset.borderColor =
              colors.datasets.primary[index % colors.datasets.primary.length];
          }
        });
