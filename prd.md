...
- **Testing:** bUnit for component tests (≥80% coverage)

### 4.2 BlazorWasm.Server (API)

- **Framework:** ASP.NET Core 9.0.6
- **Data Access:** EF Core 9.0.6 (MySQL provider); migrations applied at startup
...
- **Endpoints & Behaviors:**
  ...
  - `GET /api/tasks/export` — streams CSV with proper escaping; supports large data (>100k rows) via buffering
...
## 5. Feature Set & Acceptance Criteria

| Feature                    | Priority | Acceptance Criteria                                                                                                                    |
| -------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Authentication & Roles     | High     | Users sign in/up; JWT + refresh token issued; access denied (403) when unauthorized; lockout after 5 failed attempts in 15 min window. |
...
**6.1 Natural-Language Task Creation**
...
**6.2 Task Summarization & Contextual Insights**
...
**6.3 Priority & Deadline Recommendations**
...
**6.4 Conversational Assistant Interface**
...
**6.5 Sentiment Analysis for Comments**
...
**6.6 Automated Report & Notification Drafting**
...
- **Load Testing:** k6 script simulating 200+ concurrent users; benchmark API
